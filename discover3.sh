...
if [ "$flag_mitm" = true ]; then
    ...
    #sudo docker exec -d mitm bash -c "bettercap -iface wlp42s0 -eval \"set arp.spoof.fullduplex true; set arp.spoof.targets $ip_victima; arp.spoof on\""   arp spoof amb bettercap, afegir el apt install -y bettercap
